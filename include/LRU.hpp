...
struct cache_t
{
    ...
    cache_t(size_t size) : cache_size(size), cache_list(), hash(), misses(0), hits(0)
    {
        hash.reserve(size);
    };
...
template <typename type, typename key_type>
void cache_t<type, key_type>::resize_cache(size_t new_size)
{
    std::cout << "sizes: " << new_size << " | list size: " << cache_size << std::endl;
    if (cache_size > new_size)
    {
        std::cout << "resize down" << std::endl;
        while (cache_list.size() > new_size)
        {
    ...
    else if (cache_size < new_size)
    {
        std::cout << "resize up" << std::endl;
        auto old_back = cache_list.end();
        cache_list.resize(new_size);
        for (auto it = old_back; it != cache_list.end(); it++)
        {
            hash[*it] = it;
        }
    }
    ...
    {
        std::cerr << "cache size has already new size" << std::endl;
    }
}
...
template <typename type, typename key_type>
void ReadTests(cache_t<type, key_type>& cache)
{
    ...
    if (input_file.is_open())
    {
        while ((std::getline(input_file, data_line)) && (std::getline(input_file, cache_line)))
        {
            ...
            //std::cout << "cache test size: " << cache_test.size() << std::endl;

            //cache.resize_cache(cache_test.size());

            //std::cout << "cache size: " << cache.cache_list.size() << std::endl;
            //std::cout << "test size:  " << cache_test.size()       << std::endl;
